...
$query="SELECT DISTINCT * FROM producto INNER JOIN mediars ON mediars.ID_Producto = producto.ID INNER JOIN marcas ON marcas.ID = producto.ID_Marca WHERE marcas.ID =$idCat ORDER BY producto.Nombre";
if($res = mysqli_query($conx,$query)){
    ...
    $matri[$conta]['nombrec']= utf8_encode($row[14]);
